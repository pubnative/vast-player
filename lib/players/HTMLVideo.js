...
function HTMLVideo(container, attributes) {
...
HTMLVideo.prototype.load = function load(mediaFiles) {
    ...
    return new LiePromise(function loadCreative(resolve, reject) {
        ...
        Object.keys(self.attributes).forEach(function (key) {
            video.setAttribute(key, self.attributes[key]);
        });
        ...
        // this way muted autoplay works on latest android (8.1.0)
        self.container.innerHTML = video.outerHTML;
        video = self.container.childNodes[0];
        ...
        once(video, HTML_MEDIA_EVENTS.ERROR, function onerror() {
            var message = video.error ? video.error.message : 'Unknown video player error';

            self.emit(VPAID_EVENTS.AdError, message);
            reject(message);
        });
