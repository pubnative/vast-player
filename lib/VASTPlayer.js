...
VASTPlayer.prototype.load = function load(uri) {
    ...
    return this.setupHandler(VAST.fetch(uri, config));
};

VASTPlayer.prototype.loadXml = function load(xml) {
    var config = this.config.vast;

    var vast = new VAST(VAST.pojoFromXML(xml));
    var vastPromise = config.resolveWrappers ? vast.resolveWrappers(config.maxRedirects) : LiePromise.resolve(vast);

    return this.setupHandler(vastPromise);
};

VASTPlayer.prototype.setupHandler = function(vastPromise) {
    return vastPromise
...
VASTPlayer.prototype.setReady = function() {
    this.__private__.ready = true;
    this.emit('ready');

    return this;
};

VASTPlayer.prototype.emitError = function(reason) {
    this.emit('error', reason);

    throw reason;
};

VASTPlayer.prototype.loadPlayer = function load(vast) {
    var self = this;
    var config = (function() {
        ...
        return {
            player: new HTMLVideoPlayer(self.container, self.config.attributes),
            mediaFiles: files
        };
    }());
    ...
    return player.load(mediaFiles, parameters)
